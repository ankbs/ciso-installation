...
$instanceName = Read-Host "Bitte gib einen Namen für diese CISO-Instanz ein (z.B. 'local', 'dev', 'finance') [Standard: local]"
if ([string]::IsNullOrWhiteSpace($instanceName)) {
    $instanceName = "local"
}
$instanceName = $instanceName.ToLower().Replace(" ", "-")

# Suggest default port based on name
$defaultPort = 8443
if ($instanceName -ne "local") {
    $defaultPort = 8444
}

$port = $defaultPort
$portChoice = Read-Host "Bitte gib den HTTPS-Port für diese Instanz an [Standard: $defaultPort]"
if (-not [string]::IsNullOrWhiteSpace($portChoice)) {
    $parsedPort = 0
    if ([int]::TryParse($portChoice, [ref]$parsedPort)) {
        $port = $parsedPort
    }
}

# Custom Qdrant port to avoid collisions
$qdrantPort = 6333 + ($port - 8443)

Write-Host "[*] Selecting installation drive..." -ForegroundColor Cyan
$fixedDrives = [System.IO.DriveInfo]::GetDrives() | Where-Object { $_.DriveType -eq 'Fixed' -and $_.IsReady }

Write-Host "Verfügbare Festplatten für die Installation:" -ForegroundColor Yellow
for ($i = 0; $i -lt $fixedDrives.Count; $i++) {
    $d = $fixedDrives[$i]
    $freeGB = [Math]::Round($d.AvailableFreeSpace / 1GB, 1)
    Write-Host " [$($i + 1)] $($d.Name) ($freeGB GB frei)" -ForegroundColor White
}
Write-Host ""

$driveChoice = Read-Host "Bitte wähle die Nummer der Festplatte [1-$($fixedDrives.Count)] (Standard: 1)"
if ([string]::IsNullOrWhiteSpace($driveChoice)) {
    $driveChoice = 1
}

$choiceInt = 1
if (-not [int]::TryParse($driveChoice, [ref]$choiceInt)) {
    $choiceInt = 1
}

if ($choiceInt -lt 1 -or $choiceInt -gt $fixedDrives.Count) {
    $choiceInt = 1
}

$selectedDrive = $fixedDrives[$choiceInt - 1].Name.TrimEnd('\')
$DockerRoot = Join-Path "$selectedDrive\" "_Docker"
$LocalDir = Join-Path $DockerRoot "ciso-assistant-$instanceName"
...
$profileChoice = Read-Host "Bitte wähle die Nummer des Onboarding-Profils [1-5] (Standard: 4)"
